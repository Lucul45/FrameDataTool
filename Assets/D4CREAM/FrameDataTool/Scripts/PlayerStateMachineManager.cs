...
public class PlayerStateMachineManager : Singleton<PlayerStateMachineManager>
{
    ...
    void Start()
    {
        ...
        {
            state.Value.Init(_players[1], this, _players[0].Animator, _players[0].SpriteRenderer, _players[0].Rb, _players[0]);
        }
        foreach (KeyValuePair<EPlayerState, APlayerState> state in _statesP2)
        {
            state.Value.Init(_players[0], this, _players[1].Animator, _players[1].SpriteRenderer, _players[1].Rb, _players[1]);
        }
